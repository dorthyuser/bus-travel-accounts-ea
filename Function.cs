...
namespace GetAccountLambda
{
    public class Function
    {
        ...
        {
            var basicCorrelation = request.Headers != null && request.Headers.ContainsKey("CUSTOMER_CORRELATION_ID")
                ? request.Headers["CUSTOMER_CORRELATION_ID"]
                : "CUSTOMER_CORRELATION_ID_NOT_FOUND";
            ...
            context.Logger.LogLine($"START - Get Account Details. X_CORRELATION_ID={xCorrelation}");

            try
            {
                // Initialize accountId like Mule
                string accountId = null;
                if (request.PathParameters != null && request.PathParameters.TryGetValue("id", out var id)) accountId = id?.Trim();

                context.Logger.LogLine($"Before Request - bus-travel-accounts-sa-get-account-details. AccountId={accountId}");
                ...
                url.Append(basePath).Append("/accounts/").Append(Uri.EscapeDataString(accountId ?? string.Empty));
                ...
                context.Logger.LogLine($"After Request - bus-travel-accounts-sa-get-account-details. httpStatus={(int)resp.StatusCode}");
        ...
        private async Task WriteErrorToS3Async(object payload, APIGatewayProxyRequest request, ILambdaContext context)
        {
            try
            {
                ...
                var content = System.Text.Json.JsonSerializer.Serialize(new { payload, request = new { request.Path, request.HttpMethod } });
