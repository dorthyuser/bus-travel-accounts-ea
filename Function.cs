...
namespace GetAccountsLambda
{
    public class Function
    {
        ...
        {
            // Initialize variables similar to Mule flow
            ...
            var clientIdFromHeader = request.Headers != null && request.Headers.ContainsKey("client_id") ? request.Headers["client_id"] : null;

            // Query param 'email' expected as ?email=...
            var email = request.QueryStringParameters != null && request.QueryStringParameters.ContainsKey("email") ? request.QueryStringParameters["email"] : null;

            context.Logger.LogLine($"START - Request received - X_CORRELATION_ID:{correlationId}");
            try
            {
                // Prepare backend headers
                using var httpRequest = new HttpRequestMessage(HttpMethod.Get, "/accounts" + (string.IsNullOrEmpty(email) ? "" : "?email=" + Uri.EscapeDataString(email)));
                ...
                var clientId = _configuration["https.requester.sa.accounts.client_id"] ?? clientIdFromHeader ?? "";
                ...
                // Mirror Mule choice logic: if 200 return payload; otherwise return payload with status
            ...
            catch (Exception ex)
            {
                ...
                // Send error to S3 (as Mule does in error handler)
                try
                {
                    ...
                    var putReq = new PutObjectRequest
                    {
                        ...
                        Key = key,
                        ContentBody = errorJson
                    };
                    await s3.PutObjectAsync(putReq);
                    context.Logger.LogLine($"Error uploaded to S3 bucket {_s3Bucket} key {key}");
                }
